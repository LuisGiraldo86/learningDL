...
def reuters_model_four_layers(units:list, activ_func:str, X_train, y_train, X_val, y_val)->tuple:

    # model definition
    model = keras.Sequential([
        layers.Dense(units=units[0], activation=activ_func),
        layers.Dense(units=units[1], activation=activ_func),
        layers.Dense(units=units[2], activation=activ_func),
        layers.Dense(units=units[3], activation="softmax")
    ])
